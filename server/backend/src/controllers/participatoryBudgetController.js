...
const { createBudgetSchema, voteBudgetSchema } = require('../validators/participatoryBudgetSchema');
    ...
    try {
        ...
        if (!users[0].IS_ADMIN) {
            return res.status(403).json({ 
                timeStamp: new Date().toISOString(),
                message: "Accesso negato: solo gli amministratori possono visualizzare l'archivio storico." 
            });
        }
        ...
        // Conteggio totale
        const [countRows] = await db.query('SELECT COUNT(*) as total FROM BILANCIO_PARTECIPATIVO');
...
exports.createParticipatoryBudget = async (req, res) => {
    ...
    try {
        ...
        const [users] = await connection.query('SELECT IS_ADMIN FROM UTENTE WHERE ID_UTENTE = ?', [userId]);
        ...
        if (activeBudgets.length > 0) {
            ...
            return res.status(409).json({ 
                timeStamp: new Date().toISOString(),
                message: "Impossibile creare un nuovo bilancio: ne esiste già uno attivo.",
        ...
        const insertBilancio = `
            ...
            VALUES (?, ?, ?)
        `;
        const [resBil] = await connection.execute(insertBilancio, [userId, title, expirationDate]);
...
exports.voteParticipatoryBudget = async (req, res) => {
    ...
    try {
        ...
        `;
        const [optionsResult] = await connection.query(queryFindOption, [budgetId, position]);
